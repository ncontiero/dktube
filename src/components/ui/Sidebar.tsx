...
function SidebarRail({ className, ...props }: ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        `
          hover:after:bg-sidebar-border absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 duration-200 ease-linear
          group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2
          after:w-0.5 sm:flex
        `,
...
function SidebarSeparator({
  ...
  return (
    <Separator
      ...
      className={cn("bg-sidebar-foreground/20 mx-2 w-auto", className)}
      {...props}
    />
  );
}
    ...
    <Comp
      ...
      className={cn(
        `
          ...
          font-medium outline-hidden duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0
        `,
